    ...
    <div class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <select name="funder_type" class="form-select form-select-sm">
                ...
                <option value="private" 
                    @selected(old('funder_type', $invoice->funder_type ?? '') == 'private')>
                    Private
                </option>
                <option value="public"  
                    @selected(old('funder_type', $invoice->funder_type ?? '') == 'public')>
                    Public
                </option>
            </select>
        </div>
        ...
        <div class="col-md-6">
            ...
            <select name="funder_id" class="form-select form-select-sm">
                ...
                @foreach(\App\Models\Funder::where('is_active', true)->get() as $f)
                    <option value="{{ $f->id }}"
                        @selected(old('funder_id', $invoice->funder_id ?? '') == $f->id)>
                        {{ $f->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            <label class="form-label">Care Price (£)</label>
            <input type="number" step="0.01" name="care_price"
                value="{{ old('care_price', $invoice->total_amount ?? '') }}"
        ...
        <div class="col-md-6">
            ...
            <input type="number" name="travel_time"
                value="{{ old('travel_time', $invoice->travel_time ?? '') }}"
                class="form-control form-control-sm">
        </div>
